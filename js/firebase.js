...
async function fetchUserList() {
    ...
    querySnapshot.forEach((doc) => {
        ...
        if (userData.trackDesignActive) { // Only show users with trackDesignActive: true
...
window.showAdminDashboard = async function () {
 ...
 <div id="adminDashboard">
    ...
    <div id="designRequestPopup" class="design-popup" style="display: none;">
        <div class="popup-content">
            <h3><i class="fas fa-file-alt"></i> Design Request Details</h3>
            <div id="designRequestContent" class="design-content"></div>
    ...
    `;
    await fetchUserList(); // Fetch and display the list of users
};
...
window.signIn = async (e) => {
    ...
    try {
        ...
        await updateUserProfile(userCredential.user); // Fetch and display cart data
    } catch (error) {
        showError('loginError', error.message);
    }
};
...
window.signInWithGoogle = async () => {
    try {
        ...
        const user = result.user;
        await handleNewGoogleUser(user);
        await updateUserProfile(user); // Fetch and display cart data
    } catch (error) {
        showError('loginError', error.message);
    }
};
